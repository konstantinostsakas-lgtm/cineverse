...
const socket = io('http://http://localhost:5000:5000', { autoConnect: false });
    ...
    const fetchData = async () => {
      try {
        ...
        if (currentUserId && currentUserId !== 'guest') {
          const friendsRes = await fetch(`http://http://localhost:5000:5000/api/friends/list/${currentUserId}`);
