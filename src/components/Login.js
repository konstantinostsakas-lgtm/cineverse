...
function Login({ onLoginSuccess, currentLang = 'el' }) {
  ...
  useEffect(() => {
    const fetchAvatars = async () => {
      try {
        const response = await fetch('http://http://localhost:5000:5000/api/avatars');
        if (response.ok) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isRegisterMode) {
        ...
        const response = await fetch('http://http://localhost:5000:5000/api/register', {
          method: 'POST',
        ...
        const response = await fetch('http://http://localhost:5000:5000/api/login', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
